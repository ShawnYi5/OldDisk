...
class StorageChain(abc.ABC):
    ...
    def _query_key_storage_info_list(self):
        ...
        for i in range(storage_info_list_len):
            ...
            assert storage_info['storage_status'] != m.SnapshotStorage.STATUS_DELETED
            ...
            if storage_info['image_path'] != self._storage_info_list[i + 1]['image_path']:
                ...
                continue
            if self._storage_info_list[i + 1]['storage_status'] in m.SnapshotStorage.STATUS_WRITING:
...
class StorageChainForRead(StorageChain):
    ...
    def acquire(self):
        assert self._storage_info_list[-1]['storage_status'] != m.SnapshotStorage.STATUS_CREATING
        try:
